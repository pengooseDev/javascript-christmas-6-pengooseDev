...
- [x] 증정 메뉴(ServiceMenu)는 <총혜택 금액>에 포함하지만 <할인 후 예상 결제 금액>는 포함하지 않는다.
...
- [x] 고객에게 적용된 이벤트 내역만 보여 주세요. 적용된 이벤트가 하나도 없다면 혜택 내역 "없음"으로 보여 주세요.
...
- [x] 혜택 내역 중 증정 이벤트를 제외한 값을 합산하여 할인전 총 주문금액을 감한다.
